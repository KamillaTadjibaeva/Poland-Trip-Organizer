...
normalize_city_key <- function(x) {
  x <- chartr("ąćęłńóśźżĄĆĘŁŃÓŚŹŻ", "acelnoszzACELNOSZZ", x)
...
server <- function(input, output, session) {
  ...
  plan <- reactive({
    ...
    selected_transport <- input$transport
    if (length(selected_transport) == 0L) {
      return(structure(
        list(error = "Please select at least one transport mode."),
        class = "trip_error"
      ))
    }

    primary_transport <- selected_transport[[1]]
    ...
    tryCatch({
      ...
      if (isTRUE(input$scenic_only)) {
        d <- find_route_discoveries(
          route_cities = route_for_discovery,
